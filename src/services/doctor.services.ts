...
import { ICreateDoctorPayload, IDoctor, IDoctorUserDetails, IUpdateDoctorPayload } from "../types/doctor.types";
...
export const getDoctorById = async (id: string) => {
  try {
    const doctor = await httpClient.get<IDoctorUserDetails>(`/doctors/${id}`);
    return doctor;
  } catch (error) {
